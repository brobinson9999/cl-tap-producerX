...
purpose: consumes a symbol (representing the type of the object?) and produces a list of functions whose results must be equivalent when run against two different objects, in order for the two objects to be considered equivalent."
...
purpose: declares a function to be a test helper. Behaves similar to defun, but behind the scenes creates a macro instead which will call an inner function which has the behavior of the original function, but takes an additional raw-test keyword parameter."
  (let ((gen-name (gensym)))
    `(progn
